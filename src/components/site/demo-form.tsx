  ...
  return (
    <div className="rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(24,24,24,0.96),rgba(10,10,10,1))] p-6 shadow-[0_36px_100px_rgba(10,10,10,0.35)] md:p-8">
      <div className="mb-5 flex items-center gap-2 text-[11px] uppercase tracking-[0.24em] text-white/45">
        <span aria-hidden="true" className="h-2.5 w-2.5 rounded-full bg-[#fd5f56]" />
        <span aria-hidden="true" className="h-2.5 w-2.5 rounded-full bg-[#fdbc2e]" />
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <label className="block">
          ...
          <textarea
            ...
            className="w-full rounded-2xl border border-white/10 bg-white/3 px-4 py-4 font-mono text-sm leading-7 text-white outline-none transition placeholder:text-white/35 focus:border-[var(--color-gold)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold)]/50"
          />
        </label>
        <button
          type="submit"
          disabled={state.status === "loading"}
          className="inline-flex min-h-12 items-center justify-center rounded-lg bg-[var(--color-gold)] px-5 py-3 text-sm font-semibold uppercase tracking-[0.08em] text-[var(--color-black)] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-gold)] focus-visible:ring-offset-2 focus-visible:ring-offset-black"
        >
      ...
      <div
        role="status"
        aria-live="polite"
        className="mt-6 min-h-40 rounded-2xl border border-white/8 bg-black/30 p-4 font-mono text-sm leading-7 text-white/88"
      >
        {state.status === "idle" ? (
